...
Route::middleware('guest:admin')->group(function () {
    ...
    Route::post('admin/login', [LoginController::class, 'login'])->name('admin.login.submit')->middleware('throttle:5,1');
});
...
Route::middleware('auth:admin')->group(function () {
    ...
    // Cadastro de novos administradores restrito exclusivamente ao SuperAdmin (nível 1)
    ...
    // Gestão de estagiários (Rotas protegidas)
    ...
    Route::match(['get', 'post'], 'lista-estagiarios', [PontoController::class, 'listaEstagiariosDia'])->name('lista.estagiarios');

    // Gestão de ocorrências e eventos (Rotas protegidas)
...
// Rotas públicas do quiosque de ponto (Com proteção contra varredura/força bruta)
Route::get('inicio', [PontoController::class, 'index'])->name('inicio.index');
Route::post('registrar-ponto', [PontoController::class, 'store'])->name('registrar-ponto')->middleware('throttle:30,1');
Route::post('processar-qrcode', [PontoController::class, 'processarQrcode'])->name('processar.qrcode')->middleware('throttle:30,1');
